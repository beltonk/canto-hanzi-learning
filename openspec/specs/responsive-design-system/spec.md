...
## Purpose
Define the foundational design system primitives — canonical breakpoints, viewport/orientation/element-size React hooks, fluid Chinese typography, safe-area inset utilities, and adaptive spacing tokens — that every page and component in the app uses to render correctly across phone portrait, phone landscape, iPad portrait, iPad landscape, and desktop. The system establishes a single source of truth for responsive decisions and guarantees the Hong Kong Chinese font stack is preserved at every viewport.

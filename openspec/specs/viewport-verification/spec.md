# viewport-verification Specification

## Purpose
Define the automated end-to-end verification harness that loads every user-facing route at a fixed matrix of viewport profiles, asserts layout invariants (no horizontal scroll, no safe-area overlap, ≥44 px touch targets, Hong Kong font stack first, no inline overflow), captures visual-regression baselines, and gates pull requests on the matrix passing. The harness documents the protocol for adding routes/viewports and re-baselining screenshots.
